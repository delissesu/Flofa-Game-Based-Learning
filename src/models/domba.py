...
class Domba:
    ...
    def set_animation(self, animation_frames, animation_speed=0.11):
    ...
    def update_movement(self):
        ...
        if self.move_timer <= 0:
            ...
            if self.state == "idle":
                if rand_val < 0.5:
                    self.state = "walking"
                    self.move_duration = random.randint(80, 160)
                    self.speed = random.uniform(0.4, 0.9)
                else:
                    self.state = random.choice(["idle", "sleeping"])
                    self.move_duration = random.randint(120, 250)
            elif self.state == "walking":
                self.state = random.choice(["idle", "sleeping"])
                self.move_duration = random.randint(120, 250)
            elif self.state == "sleeping":
                self.state = "idle"
                self.move_duration = random.randint(70, 130)

            if self.state == "walking":
                self.target_pos = (
                    random.randint(max(0, self.rect.centerx - 130), min(MAP_WIDTH, self.rect.centerx + 130)),
                    random.randint(max(0, self.rect.centery - 130), min(MAP_HEIGHT, self.rect.centery + 130))
                )
            
            self.move_timer = self.move_duration

        if self.state == "walking":
            self.set_animation(self.walk_frames, animation_speed=0.13)
